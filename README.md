...
- **Pipeline** — stages, activity timeline, tasks, and follow-up dates that schedule
  ...
  notes, contacts or job descriptions. A saved view can have its own link, which is usually
  the better offer: "everything I have from a referral" is a reasonable thing to send
  someone and the whole board is not. Everything the view filters out is not in the page at
  all, and each link has its own address, so revoking one leaves the others alone.
- **Offers** — what somebody actually put on the table, which is a different fact from
  the range the posting advertised and is never mixed with it. Base, bonus, equity,
  sign-on, the vesting schedule, the terms in words, and the day you have to answer by —
  that last one goes straight into the bell, because it is the one date in this app that
  cannot be caught up on tomorrow. Recording a better number writes a new version rather
  than overwriting the old one, so the distance between the opening offer and the closing
  one stays on file: that gap is the only evidence anybody keeps that negotiating worked.
  The board prints the real number in place of the advertised range once there is one, and
  `compare_offers` puts every live one side by side, sorted by what year one actually pays.
  It will not convert currencies — it says so and asks which rate you want — and
  `offer_briefing` gathers everything on file for one negotiation in a single read: every
  ...
  still missing.
- **Two emails, if you want them** — nothing leaves this app unless you ask. Two things can:
  a summary on Monday of where the search stands, what moved, what is coming and the one
  thing worth fixing, and a nudge on a day something is actually due. The nudge sends
...
- **Stage checklists** — the things you always mean to do and never do. Say once that
  ...
  from then on. Settings → Pipeline holds them, one short list per stage, each line with a
  due date counted from the move and a switch to park it. Two rules make it trustworthy:
  a line fires once per job ever, so dropping back a stage and going forward again does
  not re-add what you already ticked off, and adding a line never reaches backwards onto
...
app reads it: anything under a heading called **Rules** is how that job may be described,
anything under **Caveats** is yours — interview prep and positioning that never reaches a
resume or a letter — and anything under **Open questions** is a fact you haven't settled yet,
which stays off every document until you answer it. One line can be marked the same way
