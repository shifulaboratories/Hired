...
- **The replace-versus-append table.** `update_role`, `update_resume`, `update_company`
  ...
  nine `update_company` calls replaces the industry list on each, dropping whatever else
  it held.
...
- **A reminder to read your notes and look for the ones marked `GUARDRAIL`**, because your
  ...
  takes a `limit`, a hundred by default, and says so when it cut the list off, so the
  picking-out happens on the way back.
...
- **An ending is a stage, not a deletion.** `LOST`, with a reason on it, keeps an application in
  the funnel, which is the only thing that can say where the search is losing people.
  Deleting takes it out — and archives it, so it is recoverable for thirty days by default.
