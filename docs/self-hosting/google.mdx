...
on the sign-in page as well, and clearing the client ID takes it away again. The same
client is what lets people connect Gmail and Calendar, though, so clearing it also greys
out the Google option under **Settings → Connections** and stops the accounts already
connected there.
...
<Steps>
  <Step title="Enable the two APIs">
    **APIs & Services → Library**: enable the **Gmail API** and the **Google Calendar
    ...
    shown on the page that asked for it, and names the project.
  </Step>
