...
description: "Nineteen tools for writing documents, checking their length before saving, comparing a tailored variant against its base, publishing them to a link and exporting real PDFs."
icon: "file-lines"
---
...
_Put a job from Me into a resume_
...
| --- | --- | --- |
| `id` | string | **required** — Resume id |
| `role` | string | **required** — The role: its id, or the company or title it goes by |
| `section` | string | Which experience section, by id or heading. Defaults to the first one — most resumes have exactly one. |
| `position` | number | Where among the jobs it lands, 1-based. Defaults to last. |
| `bullets` | number | How many of the role's highlights to bring. Default 6. |

### `reorder_resume`

_Move a section, entry or bullet_
...
| --- | --- | --- |
| `id` | string | **required** — Resume id |
| `render` | boolean | Also read the text back off the printed page in a headless browser. On by default where one exists; pass false to skip the browser and get the document half instantly |
