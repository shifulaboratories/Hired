...
description: "Eleven tools that read the threads and meetings in your own accounts — Google, Microsoft 365, or any IMAP and CalDAV provider — behind any contact, company, application or resume. Live, read-only, never copied."
icon: "envelope"
---
...
_Turn the mail sweep on or off_
...
| Argument | Type | |
| --- | --- | --- |
| `on` | boolean | **required** — True to sweep, false to stop |

### `run_mail_sweep`

_Read what is new in the mail now_
...
| Argument | Type | |
| --- | --- | --- |
| `days` | number | Look back this many days instead of picking up where the last sweep stopped. Use it for a deliberate wider look |
| `include_calendar` | boolean | Also look at booked meetings, which is where the one stage move comes from. Default true when a calendar is connected |
