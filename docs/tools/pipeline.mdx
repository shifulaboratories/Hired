...
| `interviewRound` | number | Which round this puts them all in, counting from 1. Only read when moving to INTERVIEWING. |
| `roundLabel` | string | What that round is called — 'Phone screen', 'Onsite'. Applied to all of them. |
| `lossReasons` | string[] | Why they ended, by name — the same reasons on every one. REPLACES any already on them. Only read when moving to LOST. |
...
| `interviewRound` | number | Which round this puts it in, counting from 1. Only read when moving to INTERVIEWING. |
| `roundLabel` | string | What that round is called — 'Phone screen', 'Take-home', 'Onsite'. Optional. |
| `lossReasons` | string[] | Why it ended, by name. REPLACES any reasons already on it. Only read when moving to LOST. |
