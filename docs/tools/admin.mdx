...
title: "Admin tools"
sidebarTitle: "Admin"
description: "Thirty-four tools for running an instance: accounts, invitations, the waitlist, sign-in, email, billing, health and every stored setting."
icon: "shield-halved"
---
...
| `source` | enum | Only entries from this part of the app. Omit for everything.  `stripe.webhook` · `billing.sync` · `email.send` · `google.signin` · `google.data` · `microsoft.data` · `mcp.tool` · `mcp.origin` · `outbound.send` · `app` |
...
_Check the built-in assistant_
...
_Configure the built-in assistant_
...
| Argument | Type | |
| --- | --- | --- |
| `anthropicApiKey` | string | Anthropic API key from console.anthropic.com, starts with sk-ant-. Empty turns the feature off. |
| `model` | string | Which model answers. Default claude-opus-5. |
| `dailyMessages` | number | Messages one person may send in a day. 0 means no cap. |
| `scope` | enum | Which tools it is served: FULL, WRITING, PIPELINE or READONLY  `FULL` · `WRITING` · `PIPELINE` · `READONLY` |
