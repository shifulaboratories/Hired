---
title: "The pipeline"
description: "Six stages with two endings, an activity timeline, tasks, follow-up dates that set themselves, saved views, and a read-only link for whoever is helping you."
icon: "list-check"
---
...
<Note>
  ...
  `sources` and `source` still work: tags were called sources until they grew
  to cover companies and people too. `tags` wins wherever both are passed.
</Note>

A new workspace is offered LinkedIn, Indeed, Company site, Referral, Recruiter reached
...
weeks" lives. In the app, both land on **Today**.

### Three ways to ask what is due

<CardGroup cols={3}>
  <Card title="list_follow_ups" icon="bell">
    What is already due, or due within N days. Returns three lists: applications whose date
    has arrived, contacts whose ping has, and tasks that are due or already late. All three
    are work.
  </Card>
...
company, which resume went out, how long it has sat still, and how long since anything was logged. It
...
| `f` | Comma-separated stages, plus `overdue` as a flag. `closed` expands to both endings. |
...
| `qd` | Minimum days since anything was logged |
