...
Two things depend on it, which is why an assistant is told to set it whenever it learns it.
It is what puts the company's logo on your pipeline — a board with logos reads at a glance,
and a board without them is a wall of text. And with a [mail or calendar
account](/guides/inbox-and-calendar) connected, it is the domain their threads and meetings
are matched on: `list_correspondence` reads it off `website`, so everything from that domain
counts, not only the people you have added by hand.
...
**People** in the sidebar splits into Companies and Contacts, each a filterable list. The
entry itself is a link to the CRM; the chevron beside it unfolds the two, and the tabs at
the top of either page switch between them. A company page holds its details, its research
notes, everyone who represents it, and its roles as job listings — each one a click from
the posting it came from. Industry, size, location and its own tags are pickers there: tick to
