...
<AccordionGroup>
  ...
  <Accordion title="Can I import my existing resume or LinkedIn export?">
    ...
    start date keeps everything it has and gains what it doesn't — the bullets it is
    missing become highlights, and the new wording is appended to its background under a
    dated heading. A profile field you have filled in is kept, and skill lists merge.
    Nothing is edited or removed, so importing the same document twice writes nothing the
    second time, and re-importing an updated one brings in what changed.
