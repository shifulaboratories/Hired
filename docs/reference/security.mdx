...
Every administrative action is written to the audit log with the acting admin's email
address, values included for everything that is not a secret. Secrets are recorded as having
been set, never as their value. Audit rows survive the deletion of the account they
describe.

## Connection tokens

A connection URL contains a secret token tied to **one account**. It cannot reach anyone
else's data. It can read and write all of that account's data — and where that person has
connected a mailbox or a calendar, read those too. `list_correspondence`, `search_email`,
`get_email_thread` and `search_calendar` are ordinary tools, available to anyone holding
the URL. They are read-only, and nothing they return is stored here.
...
it is connected as. It carries the five areas — Me, resumes, the pipeline, the CRM, and
mail and calendar — plus the tags that cut across them, which tool to reach for first in
each, the fact that deleting a company, a person or an application archives it and that
emptying that archive is one of only two acts on the server that cannot be undone, the fact
that connection URLs are credentials that must never be repeated where they will be
...
Your own [standing rules](/concepts/me#standing-rules) come first in that briefing, ahead of
everything but the line naming whose workspace it is, under a header saying they override
any inference the assistant would otherwise make. They share a 1,000-character budget,
header and notice included; if you have more than fits, the rest are dropped — and the
...
| Fetching a job posting page | When `capture_job_posting` is called, and when you paste a link into the new-application dialog. | Do not call it, and paste the description in by hand rather than the link. |
| Fetching an image URL | Only when `set_profile_photo` is given an `https` URL. | Pass the bytes instead. |
| Reading your mail and calendar | Only if you connected an account under **Settings → Connections**: Google, Microsoft 365, or your own IMAP and CalDAV servers. Every read is live and read-only, and nothing that comes back is stored here. | Disconnect the account. That deletes the credential, which is the only thing this instance was holding. |
