...
<AccordionGroup>
  <Accordion title="The client says it connected but no tools appear" icon="plug-circle-xmark">
    Open that connection in **Settings** and press **Test**. It calls the endpoint the way
...
<AccordionGroup>
  ...
  <Accordion title="diagnose_search gives advice that seems wrong" icon="gauge">
    The reason an application ended is not an input. `diagnose_search` reads how far each
    one ever got — its stage history and `interviewRound` — plus how many you send a week,
    how long each stage takes and which resume gets answered; a round nobody numbered
    counts as the first one, so deep interviews left unnumbered read as shallow ones.
    Reasons are what the funnel reads: `get_funnel` splits each rung's endings by their
    LOSS tag, so "Ghosted" and "Rejected" separate there rather than in the advice, and a
    `LOST` with no reason on it tells the funnel only that it stopped.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="An invitation never arrived" icon="envelope">
    `admin_list_invites` is where an unaccepted invitation lives: it carries `emailSent`,
    the exact `emailError` when the send failed, and the link to send by hand. Nobody has
    an account until they accept, so `admin_user_detail` has nothing to look up yet — that
    one is for somebody already in. [Email →](/self-hosting/email)
  </Accordion>
