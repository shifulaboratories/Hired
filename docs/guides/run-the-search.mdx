...
moves a batch to `LOST` with the reason "Ghosted", each with its own timeline entry and
its own follow-up reset,
so the funnel history stays intact.

<Warning>
  Say why it ended, and file silence as "Ghosted" rather than "Rejected". A rejection is a
  decision against you; a ghosting is a non-response. Filing one as the other — or leaving
  the reason off entirely — makes `diagnose_search` give you the wrong advice about what to
  fix.
</Warning>
