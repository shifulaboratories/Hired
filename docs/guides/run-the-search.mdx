...
so the funnel history stays intact. The reason does not come with it —
`move_applications_stage` takes ids and a stage and nothing else — so file "Ghosted" on
each one afterwards with `update_application`, or move them one at a time with
`move_application_stage`, which does take `lossReasons`.
