...
**Settings → Connections → Connect → Accounts.** Three kinds, and you can connect more than
...
If the Google or Microsoft tile is greyed out and says it needs an admin, the instance has
no client for it yet. That is an admin's job: [Google sign-in](/self-hosting/google) for
Google, which powers Continue with Google too, and a
[Microsoft app registration](/self-hosting/microsoft) for Microsoft. IMAP and CalDAV need
nothing from an admin.
...
Open a thread on any of those and it expands in place, with a link out underneath it. The
link says **Open in Gmail** whichever account the thread came from, and only goes somewhere
for Google and Microsoft — an IMAP account has no web client to link to.
